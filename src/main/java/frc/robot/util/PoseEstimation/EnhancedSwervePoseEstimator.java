...
public class EnhancedSwervePoseEstimator {
    ...
    public EnhancedSwervePoseEstimator(
        ...
        for (int i = 0; i < 3; ++i) {
            if (stateStdDevs.get(i, 0) == 0 || visionMeasurementStdDevs.get(i, 0) == 0) {
                throw new IllegalArgumentException(
                        "Deviations should not be less than or equal to 0. Ensure that the state and vision measurements deviations are greater than 0.");
            }
        }
     ...
     *     global measurements from vision less. This matrix is in the form [x, y, theta]ᵀ, with units in meters and
     *     radians.
     */
    public final synchronized void setVisionMeasurementStdDevs(Matrix<N3, N1> visionMeasurementStdDevs) {
        var r = new double[3];
        for (int i = 0; i < 3; ++i) {
            var val = clampStdDev(visionMeasurementStdDevs.get(i, 0)) * clampStdDev(visionMeasurementStdDevs.get(i, 0));
            r[i] = val;
            m_r.set(i, 0, val);
        }
    ...
    /**
     * Sets the pose estimator's trust of the state (odometry). This is useful for dynamically handling wheel slip.
     *
     * @param stateStdDevs Standard deviations of the pose estimate (x position in meters, y position in meters, heading
     *     in radians). Increase these numbers to trust odometry less.
     */
    public final synchronized void setStateStdDevs(Matrix<N3, N1> stateStdDevs) {
        // Update Q (state covariance)
        for (int i = 0; i < 3; ++i) {
            m_q.set(i, 0, clampStdDev(stateStdDevs.get(i, 0)) * clampStdDev(stateStdDevs.get(i, 0)));
        }

        // Recompute Kalman gain using existing vision noise (R)
        for (int row = 0; row < 3; ++row) {
            if (m_q.get(row, 0) == 0.0) {
                m_visionK.set(row, row, 0.0);
            } else {
     ...
     *
     * <p>The gyroscope angle does not need to be reset here on the user's robot code. The library automatically takes
     * care of offsetting the gyro angle.
     *
    ...
    public Optional<Pose2d> sampleAt(double timestampSeconds) {
        ...
        double newestOdometryTimestamp =
     ...
     *
     * <p>This method can be called as infrequently as you want, as long as you are calling {@link PoseEstimator#update}
     * every loop.
     *
     * <p>To promote stability of the pose estimate and make it robust to bad vision data, we recommend only adding
     * vision measurements that are already within one meter or so of the current pose estimate.
     *
     ...
     *     time source by calling {@link PoseEstimator#updateWithTime(double,Rotation2d,Object)} then you must use a
     *     timestamp with an epoch since FPGA startup (i.e., the epoch of this timestamp is the same epoch as
     ...
     * measurements until a subsequent call to {@link PoseEstimator#setVisionMeasurementStdDevs(Matrix)} or this method.
     *
     * @param visionRobotPoseMeters The pose of the robot as measured by the vision camera.
     * @param timestampSeconds The timestamp of the vision measurement in seconds. Note that if you don't use your own
    ...
    public synchronized Pose2d updateWithTime(
        ...
        if (wheelPositions.length != m_numModules) {
            throw new IllegalArgumentException(
                    "Number of modules is not consistent with number of wheel locations provided in the constructor. Number of modules provided in the constructor: "
                            + m_numModules + ". Number of modules provided in the method call: " + wheelPositions.length
                            + ".");
        }
    ...
    private static double clampStdDev(double val) {
        return Math.max(val, 1e-4);
    }
